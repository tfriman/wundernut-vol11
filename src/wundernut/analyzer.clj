...
(defn morse->text
  ...
  (let [input-sound                    (read-sound input-wav-path)
        ...
        hilos                          (->> (partition (/ hz 100) amplitudes)                ;; downsample to 10 ms partitions
                                            (map #(apply max %))                             ;; find max within partition
                                            (->highs amplitudethreshold))                    ;; get up&down "timestamps"
        durations                      (map (fn [[a b]] (- b a)) (partition 2 hilos))        ;; durations of dits and dashes
        durations-between              (map (fn [[a b]] (- b a)) (partition 2 (rest hilos))) ;; durations between dits and dashes
        [dashthreshold spacethreshold] (->thresholds durations-between)                      ;; calculate thresholds
        morse                          (map (fn [i] (if (> dashthreshold i) "." "-")) durations)
        word-splits                    (map (fn [i] (cond
                                                      (> i spacethreshold) " SPACE "
                                                      (> dashthreshold i) ""
                                                      :else " ")) durations-between)
...
(def morse-codes {".-.-.-" \.
                  ...
                  "-.--"   \Y
                  "--.."   \Z
                  "SPACE"  \space ;; not part of Morse code but used as word separator.
                  })
